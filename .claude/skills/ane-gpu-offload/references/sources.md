...
| `ANEQuantizedWeightProbeTests.swift` | All three weight formats run on the ANE through the in-memory path. fp16 at 0.03 max-abs. int8 affine (`constexpr_affine_dequantize`, iOS16) at 0.8 percent relative error. int4 palette (`constexpr_lut_to_dense`, iOS16) at 15.9 percent, the error of a per-tensor uniform 16-level LUT. int4 blockwise (`constexpr_blockwise_shift_scale`, iOS18) is rejected with `InvalidMILProgram` despite a byte-identical blob: the in-memory compiler accepts the iOS16 constexpr ops and not the iOS18 one. |
