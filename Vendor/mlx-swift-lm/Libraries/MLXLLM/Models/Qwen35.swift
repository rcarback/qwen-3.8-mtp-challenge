...
// MARK: - Candidate-owned M=512 seed-prefill NAX retile (qmm_t_nax_tgp_impl)
//
// Reconstruction of the charged seed-prefill experiment recorded in note
// 29da41f (Closure 3): a Swift-dispatched instantiation of MLX's own
// transposed-NAX quantized matmul, `qmm_t_nax_tgp_impl`, at candidate-chosen
// BM/BN/WM/WN geometry. The stock launcher (`quantized.cpp:473-575`) pins
// bm=64 bn=64 bk=64 wm=2 wn=2 for every shape; this retile keeps the same
// kernel body and BK=64 but widens the per-threadgroup output tile on the
// large-N shapes that dominate the 512-token seed forward.
//
...
// is unchanged, so any geometry is bit-exact vs stock; note 29da41f verified
// nDiff=0 across BM 64..512 on four shapes.
//
// Dispatch safety. `Qwen35NAXPrefill.matmul` returns nil unless every guard
...
private enum Qwen35NAXPrefill {
    ...
    /// Geometry row for one routed cell.
    /// (bm, bn, wm, wn); bk is pinned to 64 by the affine group size.
    static func geometry(k: Int, n: Int) -> (bm: Int, bn: Int, wm: Int, wn: Int)? {
        switch (k, n) {
        case (6144, 5120):
            // gdn.out_proj / fa.o_proj class: note 29da41f measured 1.15-1.18x
            return (256, 64, 4, 2)
        case (17408, 5120):
            // mlp.down_proj class: 1.14x
            return (256, 64, 4, 2)
        default:
            return nil
        }
    ...
    /// - affine 4-bit group-64 BF16 only (the crown's quantization ABI),
    /// - 2-D packed uint32 weights with w.dim(1) == k/8,
    /// - K % 64 == 0 (stock NAX gate) and additionally K % 512 == 0 so the
    ///   affine group grid aligns with the loader's 64-wide columns,
    /// - N % 64 == 0 (aligned_N true; no store_safe tail),
    /// - B == 1 (unbatched wrapper), x densely foldable [1,M,K] or 2-D,
    /// - M == 512 (the charged seed-prefill cell; full tiles only),
    ...
    static func routable(
        ...
        guard k % 512 == 0, n % 64 == 0 else { return nil }
        let m = x.dim(-2)
        guard m == 512 else { return nil }
        guard let geom = geometry(k: k, n: n) else { return nil }
        guard m % geom.bm == 0 else { return nil }
    ...
    public static func matmul(
        ...
        guard let geom = geometry(k: cell.k, n: cell.n) else { return nil }
...
func qwen35RoutedQuantizedMM(
    ...
    if let y = Qwen35NAXPrefill.matmul(
        x, w, scales: scales, biases: biases,
        groupSize: groupSize, bits: bits, mode: mode)
    {
        return y
