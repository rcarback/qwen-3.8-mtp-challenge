...
private let qwen35PackedGDNPreworkKernel: MLXFast.MLXFastKernel = {
    ...
    let source = """
        ...
        // T < NKeep: surviving prefix of the new conv state still lives in
        // the old conv state. The qkv copy above only writes the suffix.
        // Row 0 owns that prefix so T=2 (and T=1) is defined.
...
final class Qwen35GatedDeltaNet: Module {
    ...
    func callAsFunction(
           ...
           let midKernel = qwen35GatedDeltaMidKernel
        {
            ...
            let convInput = concatenated([convState, qkv], axis: 1)
            let nKeep = convKernelSize - 1
            // Width-2 verify prework through the packed mixer. T=2 < NKeep=3
            // uses the prefix conv-state copy. processChunk S=1 is not opened.
            let mixerHit = MLXHardwareInfo.isCompiledDecodeSupported
                && B == 1 && S == 2 && nKeep == 3
                && numKHeads == 16 && numVHeads == 48
                && headKDim == 128 && headVDim == 128
