...
// selects BM=128/WM=2 when an input-general tile-cost rule predicts enough
// weight-tile reuse without starving the GPU of threadgroups. Keeping WM/WN
// at MLX's shipped 2x2 topology limits the kernel to four simdgroups.
//
...
private enum Qwen35NAXRetile {
    ...
    /// weight tile. BM=128 halves that replication when there are enough rows,
    /// while the shipped WM=2/WN=2 topology keeps four simdgroups per group.
...
private let qwen35NAXTMMKernel = MLXFast.metalKernel(
    name: "qwen35_custom_affine4_g64_qmm_t_nax_retile_v2",
    inputNames: ["w", "scales", "biases", "x"],
    outputNames: ["y"],
