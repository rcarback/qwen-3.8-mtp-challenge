...
/// false` never reads.
///
/// `tier` selects the widths this entry point carries. `nil` emits all eight,
...
public enum Qwen35CustomQMV {
    ...
    /// through the tiered plan row `(2, 2, rps)`.
    ...
    public enum Table: String, Sendable, CaseIterable {
        ...
        /// silent no-op round rather than a compile error. M=2 rides tier 2:
        /// the pair kernel it replaces launched two X-groups of which one
        /// returned before any read.
    ...
    /// branch inlined into it, so one switch over all eight widths charges
    ...
    public static let grid: Grid = {
        ...
        // Default `.tight`: launch only the ceil(m / ipg) groups that do
        // work. This reconciles the restored tiered layer (morganmcg1,
        // 60d5b34) with the promoted tight-grid launch (jungjipdo, 8849fad):
        // removed groups write nothing, so the geometry is bit-identical to
        // the full-M launch while dropping its 67-80% no-op threadgroups for
        // every routed width 3..9.
        guard let raw, let parsed = Grid(rawValue: raw) else { return .tight }
        return parsed
    }()
    ...
    /// leg that reaches all eight routed widths must still report the same two
    ...
    ///
    /// E120 rung 5d measured the complete grid of the eight shapes that make up
