...
private enum Qwen35NAXRetile {
    ...
    /// Route only when the wider tile cuts M-tile groups by at least 2x, leaves
    /// at least 64 total threadgroups for occupancy, and has enough K iterations
    /// to amortize the wider-group setup. These are tile-work properties, not
    /// model-layer or request-length identities.
    static func geometry(
        ...
        guard m >= bm, k > 0, n > 0, k % bk == 0, n % bn == 0 else {
            return nil
        }
