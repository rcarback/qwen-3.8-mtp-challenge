...
private let qwen35NaxQMVHeader = """
    ...
    //   x [M, K] bf16 (M <= 16), w [N, K/8] u32, scales/biases [N, K/64] bf16 -> y [M, N] bf16
    ...
    // on the cooperative-tensor API; A rows are two activation rows per lane (fm and
    // fm + 8), so one tile carries M <= 16; rows >= M are zero.
    //
    ...
    inline int qmv(
        ...
        const uint sg,
        const uint lane,
        threadgroup float* red)        // KS * 16 * NB floats
    {
        ...
        const int aRow2 = aRow + 8;
        const int xOff = isNax ? (fn / 4) * 16 : fbC * 8;
        const bool rowLive = aRow < M;
        const bool rowLive2 = aRow2 < M;
        const bool twoRows = M > 8;     // uniform across the simdgroup

        float acc[NV];
        float acc2[NV];
        for (int i = 0; i < NV; i++) { acc[i] = 0.0f; acc2[i] = 0.0f; }
        ...
        if (isNax || isFb) {
            ...
            for (int gi = 0; gi < gPer; gi++) {
                ...
                if (rowLive2) {
                    load_x16(x + aRow2 * K + kBase + xOff, xv2);
                    chunk_sums(xv2, xsE2, xsF2);
                } else {
                    for (int i = 0; i < 16; i++) xv2[i] = bfloat16_t(0.0f);
                }
                ...
                if (twoRows) {
                    xsE2 += simd_shuffle_xor(xsE2, 1);
                    xsE2 += simd_shuffle_xor(xsE2, 8);
                    xsF2 += simd_shuffle_xor(xsF2, 1);
                    xsF2 += simd_shuffle_xor(xsF2, 8);
                }
                ...
                if (isNax) {
                    ...
                    // C elements: frag f elements 8f..8f+3 (row fm), 8f+4..8f+7 (row fm+8).
                    for (int f = 0; f < TN; f++) {
                        for (int e = 0; e < 4; e++) {
                            const int i = 4 * f + e;
                            ...
                            if (twoRows) {
                                acc2[i] = fma(s_, ct_c[8 * f + 4 + e], acc2[i]);
                                acc2[i] = fma(MODE == 0 ? 8.0f * s_ : -128.0f * s_, xsF2, acc2[i]);
                                acc2[i] = fma(b_, xsE2, acc2[i]);
                            }
                        }
                    }
                } else {
                    ...
                    for (int i = 0; i < NV; i++) {
                        ...
                        acc[i] = fma(b_, xsE, acc[i]);
                        if (twoRows) {
                            acc2[i] = fma(s_, ct_c[NV + i], acc2[i]);
                            acc2[i] = fma(MODE == 0 ? 8.0f * s_ : -128.0f * s_, xsF2, acc2[i]);
                            acc2[i] = fma(b_, xsE2, acc2[i]);
                        }
                    }
            ...
            // Publish this simdgroup's partial: red[sg][row][cN[i]].
            for (int i = 0; i < NV; i++) {
                red[(int(sg) * 16 + aRow) * NB + cN[i]] = acc[i];
                red[(int(sg) * 16 + aRow2) * NB + cN[i]] = acc2[i];
            }
        } else {
            // Unsupported layout: zero this simdgroup's partial tile.
            for (int idx = li; idx < 16 * NB; idx += 32) red[int(sg) * 16 * NB + idx] = 0.0f;
        }
        ...
        for (int idx = tid; idx < 16 * NB; idx += 32 * KS) {
            const int m = idx / NB;
            const int c = idx - m * NB;
            if (m < M) {
                float s = 0.0f;
                for (int k = 0; k < KS; k++) s += red[(k * 16 + m) * NB + c];
...
private let qwen35NaxQMVKernel = MLXFast.metalKernel(
    ...
    source: """
        ...
        const int qmv_k = x_shape[x_ndim - 1];
        const int qmv_n = w_shape[0];
        threadgroup float qnax_red[KS * 16 * 16 * TN];
...
public enum Qwen35NaxQMV {
    ...
    /// Widths 2...16: one 16-row activation tile, two rows per lane.
    static func matmul(
        _ x: MLXArray, _ w: MLXArray, scales: MLXArray, biases: MLXArray,
        m: Int, k: Int, n: Int
    ) -> MLXArray? {
        guard m >= 2, m <= 16, let geo = geometry(n: n, k: k) else { return nil }
...
public enum Qwen35CustomQMV {
    public enum Arm: String, Sendable {
        ...
        /// `Qwen35NaxQMV`: the tensor-unit kernel for widths 2...16, with the
