...
public enum ANERuntime {
    ...
    public static func cls(_ name: String) -> AnyClass? { NSClassFromString(name) }
    ...
    public static func send(_ r: AnyObject?, _ s: Selector, retained: Bool = false) -> AnyObject? {
    ...
    public static func send(_ r: AnyObject?, _ s: Selector, _ a: AnyObject?, retained: Bool = false) -> AnyObject? {
        let f = unsafeBitCast(raw, to: Send1.self)
        guard let u = f(r, s, a) else { return nil }
        return retained ? u.takeRetainedValue() : u.takeUnretainedValue()
    }
    public static func send(_ r: AnyObject?, _ s: Selector, _ a: AnyObject?, _ b: AnyObject?, _ c: AnyObject?, retained: Bool = false) -> AnyObject? {
        let f = unsafeBitCast(raw, to: Send3.self)
        guard let u = f(r, s, a, b, c) else { return nil }
        return retained ? u.takeRetainedValue() : u.takeUnretainedValue()
    }
    public static func sendUInt64(_ r: AnyObject?, _ s: Selector) -> UInt64 {
        unsafeBitCast(raw, to: SendU64.self)(r, s)
    }
    public static func sendBoolQoS(_ r: AnyObject?, _ s: Selector, qos: Int, options: AnyObject?, error: inout NSError?) -> Bool {
