...
The objective is lower request latency while preserving required model behavior.
Using the ANE matters only when it improves that outcome, including compilation,
fallback, memory use, and time spent waiting between engines.

The second pass supports keeping the dense int8 split as a local prefill option.
It does not justify an automatic load-dependent policy or more bank infrastructure.

| decision | evidence and limit |
| --- | --- |
| Keep the direct dense int8 split as an optional local path | Fraction 0.5 reached 145.5 tok/s against controls of 124.8 and 128.2. Perplexity was close on six prompts. |
| Keep GPU-only as the MoE control | Shared-expert and projection lanes lost or remained within control drift. |
| Stop extending the tested Core ML bank | Splitting packages restored ANE placement but did not recover throughput. The bottleneck remains unisolated. |
| Separate int4 bandwidth work from ANE bank work | The direct int4 prefix is 2.27 times faster than fp16 at S=1024, but measured int4 quality is worse. Investigate traffic reductions without assuming another bank is needed. |
| Stop the tested MoE micro-batch design | At 7.5k tokens, restructuring alone lost 10 percent. ANE work increased the loss. |
| Prefer existing GPU and serving follow-ups | Investigate MoE MTP startup, prefix reuse, request timeout, and prefill chunk size before adding offload machinery. |

These are local results on an M4 Max with 128 GiB and macOS 26.5.2.
A ranked gain requires the Qwen-MTP decode benchmark and its correctness gates,
within the permitted editable surface.
The tables retain the experiments, including failed engagement and noisy controls.
...
int4 group 64, with an offset, and without one. The tested Core ML forms cannot consume these GPU quantized tensors directly.
These split designs need another weight representation, with additional storage
and possible numerical error. This does not rule out future compiler support.
...
The small projection gains about 9 percent from int4 despite a fourfold reduction in
nominal weight storage. This alone cannot distinguish dispatch, compute, decompression,
or memory traffic as the limiting cost. The chained scale costs 2.9 times as much.
The measured output-side `mul` had no detectable cost in this probe.
...
| `738`, `2u9`, `2t6`, `xgy`, `lk9`, `ptc`, `afu`, `bog`, `4cr`, `azl`, the launch-cost family | Earlier attribution assigned 33 ms of a 57 ms step to launches without isolating host and GPU time; whole-step compile is blocked by the host n-gram gather | The whole-dense-layer ANE program is the one launch consolidation that does not need the compile. | Closed, unchanged. Bead `i6v` cross-references it. |
...
Both towers use the following instruments, one model-holding process at a
...
the first control. Two controls show drift but do not bound measurement noise.
...
Compressed forms add about 2.5 percentage points over fp16 in this sweep.
The isolated prefix gains are larger, as the bandwidth section explains.
Decode does not use this lane.

The fraction-0.5 arms slowed on later prompts. Thermal throttling is a possible explanation,
but this sweep did not isolate it. The later sweep adds a per-prompt gap.
Programs also failed at the second bucket: these results include GPU fallback.
The later bank experiments address the count limit but fail to improve throughput.

## Dense tower quality

The per-layer verify run logs
...
Early character differences establish output divergence. They do not establish a near tie
without tokenization and logit-gap evidence. The control repeat matched within this sweep,
but later controls differed across runs.
...
The int8 arms have similar aggregate perplexity on these six prompts.
That is limited quality evidence, not proof of unchanged behavior. The int4 form with one codebook per tensor costs 3.7 percent of
...
closing that gap, but the later bank measurements still show a perplexity increase.

The dense int8 arm is the strongest candidate in this initial sweep.
The later gapped sweep measures fraction 0.5. Neither sweep replaces the required correctness gates.
...
A disk-full incident invalidated early quality runs because programs failed to stage
and layers fell back to the GPU. The reported reruns include program build counts.
...
needs better quality evidence before adoption.
...
The int8 arms have similar perplexity on this sample. The per-tensor int4 costs 5 percent on
...
**MoE verdict.** Stop expanding these offload designs on the evidence here.
The tested arms provide no clear throughput gain, and int4 raises perplexity.
Close aggregate int8 perplexity does not establish lossless output.
Beads `wok` and `e2l` close on these measurements.

## Reading the measurements

The same-box mlx-serve ladder below supersedes the earlier release-note comparison.
Its weight packs, token counts, KV precision, cache reuse, and timers still differ.
It identifies follow-up work but does not attribute a runtime-only speedup.

Dense GPU controls produced three internally consistent completion families across the evening.
Different families diverged at the first or second token, with the cause unresolved.
Agreement comparisons require matched runs. A character difference is insufficient evidence
for a near tie. Similar perplexity is insufficient evidence for token fidelity.

Per-layer timings insert evaluations and change scheduling. Their shares describe the
instrumented execution, not an additive cost model for the normal forward.
...
The isolated split beats its GPU control in some S=512 and S=1024 cells.
GPU controls vary from 41 to 64 ms at S=1024, and the test reports best-of-nine times.
Read these ratios as exploratory shape measurements, not a fraction-selection policy.
They do not test the 128-token threshold or prove why full-model timing differs.
...
Under this instrument, the MLP accounts for about two thirds of prefill time.
The added evaluations remove overlap and can shift costs between sublayers.
The difference from uninstrumented throughput is consistent with a scheduling effect,
but does not isolate its cause or justify converting the gain into milliseconds per layer.

## Int4 and bandwidth: What was saved, and what remains untested

The direct-prefix measurements do contain a gain close to the proposed 2.4 times.
The comparison baseline matters:

| S=1024 direct prefix | time | speedup versus fp16 | speedup versus int8 |
| --- | --- | --- | --- |
| fp16 | 33.89 ms | 1.00 | |
| int8 | 18.92 ms | 1.79 | 1.00 |
| int4 | 14.96 ms | 2.27 | 1.26 |

Four-bit storage uses one quarter of fp16's nominal weight bytes, excluding metadata.
The latency benefit depends on the limiting resource. A pure weight-bandwidth limit
with equal effective bandwidth would permit a fourfold gain. Unchanged work reduces it.
For a serial fraction `p` accelerated by 2.4, the total speedup is
`1 / ((1 - p) + p / 2.4)`. Accelerating half the time gives a total speedup of 1.41.
The split also adds concurrency, so its completion time depends on the slower engine
plus preparation and joining costs. Neither formula assigns this lane a measured `p`.

The production dense GPU MLP is already q4. This is not a comparison against an
all-fp16 GPU MLP. `ANEFusedSplitMLP.swift` slices the gate/up output channels and the
down input channels, and the GPU computes only the remaining channels.
Offloading removes that prefix from GPU computation. It also creates another
weight representation for the ANE while retaining weights needed by other paths.
Lower GPU work, lower total resident memory, and lower shared DRAM traffic are separate claims.

For a simple one-read payload model, let `f` be the offloaded fraction of a q4 MLP.
Relative weight bytes are `(1-f) + f*(b_ANE/4)`, before metadata and activation traffic.
At `f=0.3125`, that is 1.94 for fp16, 1.31 for int8, and 1.00 for int4.
Int4 avoids the extra nominal payload of the higher-precision ANE copies. It does
not quarter the weight payload relative to the already-q4 GPU control.
Actual traffic can differ because of caches, repeated reads, unpacking, and staging.
Zero-copy output removes a copy but does not eliminate memory reads and writes.

Bandwidth-saving work has been tried elsewhere: dense bf16-to-q8 conversion, n-gram
quantization and layout, and expert gather measurements. `DecodeBandwidthTests.swift`
estimates effective GB/s from logical weight bytes divided by elapsed time.
Those estimates are useful but are not hardware measurements of DRAM traffic.
The older bf16 byte inventory also needs updating for the current q8 tree.

| remaining approach | smallest useful next check | existing follow-up |
| --- | --- | --- |
| Quantize selected q8 dense families to q4 | Inventory current bytes per token, then test one family with sensitive families kept q8 and held-out quality checks. | `mc6` |
| Reduce long-context KV reads and storage | Measure actual cache types and bytes at long context, with QSA and rollback engaged. | `asj` |
| Reduce n-gram pages touched | Correct boundary-page counting before testing co-occurrence layout. Prefetch alone overlaps I/O and need not reduce bytes. | `0e5`, `9ad` |
| Remove intermediate writes, repeated weight reads, and copies | Trace the current forward, then test one existing fusion or layout toggle with matched shapes and quality. | `3dh`, `qex` |

Before another ANE carrier, extend the existing probes to record nominal weight bytes,
metadata, activation sizes, and any available memory-traffic counters beside timing.
Keep storage-derived GB/s labeled as an estimate when counters are unavailable.
Compare fp16, int8, and int4 at the same shape and split fraction, then validate
full-request latency and quality. A smaller representation is useful only if it
reduces the limiting cost without failing the required behavior checks.
...
pay under this cost estimate, and bead `i6v` closes.
The external per-layer estimate uses a different run and does not prove a GPU-only remedy.
...
Source structure suggests kernel fusion and scheduling as candidates.
Operation counts and the earlier cross-run latency gap do not prove dispatch is the cause.
The existing `qex` follow-up owns controlled ablations. The estimated 1.7 ms is not a measured ceiling.

## Dense tower re-run with a cool gap per prompt

The first dense sweep gated the box only between arms, and the
fraction-0.5 arms slowed through their six prompts. This re-run waits 45
...
With a gap between prompts, int8 at fraction 0.5 beat both controls.
That supports keeping this static setting as a local option.
The experiment did not isolate ANE temperature from other changes in host activity.
The bank lost throughput. Its placement and quality corrections follow.
...
int8's does not. The speed question is separate. The bank loses even after placement is corrected.
Defer a new direct-path codebook variant unless int8 fails a concrete memory requirement.

### Bank placement correction

The first 64-function bank ran on the GPU. The initial explanation blamed the number
of codebooks, but the later package-size probes below refuted that explanation.
Single-layer per-row programs and 32-function packages were ANE-eligible.
Use the later split-bank result to judge this design.
...
At S=1024, int8 is 1.79 times faster than fp16 and int4 is 2.27 times faster.
These are direct-prefix latency ratios. These timings leave the memory-traffic explanation unresolved.
Counters are needed to distinguish external reads from internal transfers.
The earlier explanation based on weights being read again for each spatial tile remains a hypothesis.
The later split-bank result also rules out GPU placement as the sole explanation for its loss.
...
where fp16 sat at 0.2 to 0.7. This supports a benefit from compressed weights but does not identify the memory level
or establish an optimal split fraction. The output-side scale costs nothing measurable. The void rows of the first
...
With 12 percent control drift and comparisons that change sign between controls,
this sweep cannot establish a reliable loaded-box gain.
The final prompts suggest a hypothesis about sustained performance, but arm order and
host activity remain confounded. Requested GPU duty was 40 percent, with achieved duty
unmeasured in this sweep. Do not use these results to automate fraction selection.
...
Fraction 0.625 beat both controls in this sweep. Fraction 0.75 was slower.
This makes 0.625 a candidate for repeated loaded-box measurements, not a proven optimum.
The controls drifted 19 percent, and earlier fraction-0.5 results came from another sweep.

Program-limit failures varied between arms, with some layers falling back to the GPU.
These are measurements of partially engaged paths. Full engagement could change both
throughput and quality, so fallback does not make the reported gains conservative by definition.
...
Host activity and excluded stalls prevent a reliable ranking of these int4 fractions.
The fraction-0.5 rerun gained much less than the earlier filtered run.
Neither result establishes an optimum or makes 0.625 a safer default.
The int4 perplexity regressions remain, regardless of speed.
The bank's GPU placement is investigated below.
...
For the inspected function, 32-function packages used the ANE while the 64-function package used the GPU.
Function count and package size changed together, so this does not isolate a compiler limit.
The loader was extended to split banks into parts by layer range.
The following run tests that design end to end.

The per-row bank ran as two 32-layer parts, 63 of 64 bank
...
The split bank remained substantially slower than the later GPU control despite ANE placement.
Stop extending this bank design. The large control drift limits the precise loss estimate.
These runs do not isolate dispatch, data handling, package loading, or resource contention.
A universal explanation is unnecessary to reject this design.

Per-row codebooks improved int4 perplexity to 5.722, still above the 5.566 control.
A direct-path grouped-LUT design adds work without an established quality advantage over int8.
Defer that extension unless a concrete memory requirement makes int8 unsuitable.
...
about 1.4 to 1.7 times ours (22.3 against 13.3 short, 19.7 against 14.4 at 4k,
...
253 at 128k where ours does not finish. QSA and split-K kernels are candidate explanations. This ladder does not isolate them,
and prefix-cache reuse complicates the prefill comparison. Third, they
...
- the decode step (their 22 against our 13 motivates a matched comparison,
  including pack, KV, tokenizer, and timing differences)
- prefix reuse across requests
- the long-context prefill arms
- MTP on the MoE

Use the existing beads for these follow-ups. Measure fresh paired controls for each change,
rather than treating historical throughput numbers as acceptance thresholds.
