...
func buildProgram(C: Int, S: Int, layers: Int, w8a8: Bool, wBytes: Data) -> Data {
    ...
    for l in 0 ..< layers {
        let wn = "w\(l)"
        let wonly = ProcessInfo.processInfo.environment["CV_WONLY"] == "1"
        let cxq = ProcessInfo.processInfo.environment["CV_CXQ"] == "1"
        if cxq {
            // maderix's exact weight path: constexpr_affine_dequantize. int8
            // stored, expanded to fp16 at COMPILE time, so the conv sees fp16
            // but the on-disk/streamed weight is one byte. inputs are const
            // operands (quantized_data, zero_point, scale, axis).
            // constexpr operands are ATTRIBUTES (immediate values), not input
            // bindings. scale/zero_point are per-output-channel vectors [C].
            var scBytes = Data(); for _ in 0..<C { scBytes += halfBytes(0.0004) }
            let zpBytes = Data(repeating: 0, count: C)
            var cx = strF(1, "constexpr_affine_dequantize")
            cx += lenF(3, namedValue(wn, .fp16, [C, C, 1, 1]))
            cx += mapEntry(5, key: "name", value: stringValue("cx\(wn)"))
            cx += mapEntry(5, key: "quantized_data", value: tensorValueBytes(.int8, [C, C, 1, 1], wBytes))
            cx += mapEntry(5, key: "zero_point", value: tensorValueBytes(.int8, [C], zpBytes))
            cx += mapEntry(5, key: "scale", value: tensorValueBytes(.fp16, [C], scBytes))
            cx += mapEntry(5, key: "axis", value: intScalarValue(0))
            ops += lenF(3, cx)
        } else if wonly {
            // Weight-only int8: int8 stored weight -> dequantize to fp16, fp16
            // activation, NO activation quant. This is maderix's
            // constexpr_affine_dequantize / the paper's "int8 compile flag":
            // weight bandwidth halved, compute stays fp16. Stays on ANE.
...
func run() async {
    ...
    let wonly = ProcessInfo.processInfo.environment["CV_WONLY"] == "1"
    let int8weight = w8a8 || wonly || (ProcessInfo.processInfo.environment["CV_CXQ"] == "1")

    let cinE = env("CV_CIN", C), coutE = env("CV_COUT", C)
    let wCount = (env("CV_TILES", 0) > 0) ? cinE * coutE : C * C
    var w = Data(count: wCount * (int8weight ? 1 : 2))
    w.withUnsafeMutableBytes { r in
        if int8weight { let p = r.bindMemory(to: Int8.self); for i in 0 ..< C*C { p[i] = Int8.random(in: -127...127) } }
    ...
    do {
        ...
        print(String(format: "CONVPOINT\t%@\t%d\t%d\t%d\tANE=%d/CPU=%d/GPU=%d\t%.4f\t%.3f",
                     wonly ? "w8only" : (w8a8 ? "w8a8" : "fp16"), C, S, layers, aneCount, cpuCount, gpuCount,
